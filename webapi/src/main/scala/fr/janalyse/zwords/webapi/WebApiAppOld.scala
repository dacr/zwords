...
//import fr.janalyse.zwords.gamelogic.{Board, Game, GameDictionaryIssue, GameInternalIssue, GameInvalidUUID, GameIssue, GameStorageIssue, GoodPlaceCell, NotUsedCell, WrongPlaceCell}
...
//object WebApiAppOld extends ZIOAppDefault {
...
//
//  def playerCreate(playerCreate: PlayerCreate): ZIO[GameEnv, GameIssue | GameInternalIssue | PlayerIssue | WordGeneratorLanguageNotSupported, GameState] =
...
//      } yield GameState.fromPlayer(player)
//    }
//
...
//                    .getPlayer(sessionId = uuid)
//                    .some
//                    .mapError(_ => GameNotFound(playerUUID))
//      } yield PlayerInfo(
//        pseudo = player.pseudo,
//        createdOn = player.createdOn,
//        stats = PlayerSessionStatistics.fromStats(player.stats)
//      )
//    }
//
...
//    }
//
//  def renewPlayerGame(playerUUID: UUID, playerBefore: StoredPlayerSession, today: OffsetDateTime): ZIO[GameEnv, GameIssue | GameInternalIssue | PlayerIssue | WordGeneratorLanguageNotSupported, StoredPlayerSession] =
//    for {
...
//    } yield updatedPlayer
//
//  def gameGet(playerUUID: String): ZIO[GameEnv, GameIssue | GameInternalIssue | PlayerIssue | WordGeneratorLanguageNotSupported, GameState] =
//    ZIO.logSpan("gameGet") {
//      for {
//        uuid         <- extractPlayerUUID(playerUUID)
//        _            <- ZIO.log(s"player $playerUUID")
//        playerBefore <- PersistenceService.getPlayer(sessionId = uuid).some.mapError(_ => GameNotFound(playerUUID))
...
//      } yield GameState.fromPlayer(playerAfter)
//    }
//
//  val gameGetEndPoint =
//    endpoint
...
//
//  def gamePlay(playerUUID: String, givenWord: GameGivenWord): ZIO[GameEnv, GameIssue | GameInternalIssue | PlayerIssue | WordGeneratorLanguageNotSupported, GameState] =
//    ZIO.logSpan("gamePlay") {
//      for {
//        uuid          <- extractPlayerUUID(playerUUID)
//        _             <- checkGivenWordInput(givenWord)
//        player        <- PersistenceService.getPlayer(sessionId = uuid).some.mapError(_ => GameNotFound(playerUUID))
//        today         <- Clock.currentDateTime
...
//                           .mapError(th => GameStorageIssue(th))
//        state          = GameState.fromPlayer(updatedPlayer)
